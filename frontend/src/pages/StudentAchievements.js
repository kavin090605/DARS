...
const StudentAchievements = () => {
    ...
    const fetchAchievements = async () => {
        try {
            const res = await axios.get('http://localhost:5000/api/student/achievements', {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:5000/api/student/achievements', form, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
            setShowForm(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/student/achievements/${id}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
            fetchAchievements();
